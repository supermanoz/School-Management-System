...
public class AcademicServiceImpl implements AcademicService {
    ...
    @Override
    public List<AcademicPojo> getByUserIdAndGrade(Long userId, GradeEnum grade) {
        List<AcademicDto> academicDtos=academicRepository.getByUserIdAndGrade(userId, grade.name());
        SmsResponse res=webClientBuilder.build().get()
                .uri("http://USER-SERVICE/api/users/fetch/"+userId)
                .headers(headers -> {
                    headers.set("SMS_API_KEY","thisisasecret");
        ...
        academicDtos.forEach(academic->{
            academicRes.add(AcademicPojo.builder()
                    ...
                    .marks(academic.getMarks())
                    .date(academic.getDate())
                    .term(TermEnum.valueOf(academic.getTerm()))
                    .grade(GradeEnum.valueOf(academic.getGrade()))
                    .user(userRes)
                    .build()
            );
        });
